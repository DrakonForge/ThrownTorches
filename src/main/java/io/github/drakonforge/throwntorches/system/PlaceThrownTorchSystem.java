...
public class PlaceThrownTorchSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float v, int i, @NonNullDecl ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        if (currentBlock == null || belowBlock == null || currentBlock.getMaterial() != BlockMaterial.Empty || !BlockPlacementHelper.testSupportingBlock(belowBlock, 0, 0)) {
            return;
        }

        // Attempt to place block
        boolean result = BlockHelpers.setBlock(world, pos.x, pos.y, pos.z, BlockHelpers.getBlockId(stack.getBlockKey()));
        if (!result) {
            return;
        }
